...
public class BaseRemarkTemplate extends BaseController {
     ...
     */
    public Result saveRemarkTemplate(HttpSession httpSession, HttpServletRequest httpServletRequest, String title, Integer lineNumber, RemarkTemplate remarkTemplate) {
        ...
        try {
            ...
            for (int i = 0; i < lineNumber; i++) {
                ...
                String[] remarkTemplateItemsOptions = null;
                try {
                    remarkTemplateItemsOptions = httpServletRequest.getParameter(remarkTemplateItemParamName + index + itemOptionParamName).split("，");
                } catch (Exception e) {
                    result.setMsg("评语选项输入不合法");
                    return result;
                }
            ...
            result.setMsg("模版修改成功");
        } catch (Exception e) {
            result.setMsg("模版修改失败");
        }
