...
public class StudentScheduleController extends BaseController {
    ...
    public String writerSchedule(HttpSession httpSession, ModelMap modelMap) {
        //获取当前用户
    ...
    public String addSchedule(HttpSession httpSession) {
        Student student = studentService.findById(CommonHelper.getCurrentActor(httpSession).getId());
    ...
    public Result addScheduleContent(String content, Integer scheduleId, HttpServletResponse httpServletResponse) {
    ...
    @RequestMapping("/displayAllSchedule")
    @ResponseBody
    public List<Schedule> displayAll(HttpSession session){
        //GraduateProject graduateProject = graduateProjectService.findById(projectId);
        Student student = studentService.findById(CommonHelper.getCurrentActor(session).getId());

        List<Schedule> schedules = student.getGraduateProject().getSchedules();
        return  schedules;
    }
