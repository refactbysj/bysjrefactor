    ...
    <script type="text/javascript">
        ...
        function backSchoolExcellent(projectId) {
            $.messager.confirm("提示","确认驳回？", function(r){
                if(r){
                    $.ajax({
                        url: '${basePath}director/backProject.html',
...
//                            $("#projectOperation" + projectId).html("<a id='projectOperation"+projectId+"' onclick='passSchoolExcellent(" + projectId + ")'><button>推优</button></a>");
                            $('#scheduleGraduateProjectList').datagrid('reload');
        ...
        function passSchoolExcellent(projectId) {
            $.messager.confirm("提示","确认推优？", function(r){
                if(r){
                    $.ajax({
                        url: '${basePath}director/passProject.html',
                        data: {"projectId": projectId},
                        dataType: 'json',
                        type: 'post',
                        success: function () {
...
//                            $("#projectOperation" + projectId).html("<a id='projectOperation"+projectId+"' onclick='backSchoolExcellent(" + projectId + ")'><button>驳回</button></a>");
        ...
        $(function () {
            scheduleGraduateProjectListGrid = $("#scheduleGraduateProjectList").datagrid({
...
//                            if(row.commentByTutor.getTotleScoreTutor()==null
//                                    || row.commentByReviewer.getTotalScoreReviewer()==null
//                                    ||row.commentByGroup.getTotalScoreGroup()==0.0)
//                                return '<p title="指导老师、评阅人和答辩组长必须全部评分才能显示分数" style="color: red">未评分</p>'
//                            return row.getTotalScores();
                        }
                    },
...
<body>
<table id ="scheduleGraduateProjectList" style="height: 100%"></table>
<div id ="detailWindow">
